...
class lepSelection:
    ...
    def isSel(self,lep):
        if (
            lep.PT > self.pt_min
            and abs(lep.Eta) < self.eta_max
            and abs(lep.IsolationVar) < self.iso_max
        ):
            return True
        return False
...
def lepton_single(el_branch,mu_branch):
    n_el = el_branch.GetEntries()
    n_mu = mu_branch.GetEntries()
    if (n_el==1 and n_mu==0):
        return el_branch
    if (n_el==0 and n_mu==1):
        return mu_branch
    else:
        return None